...
def extract_thumb_index_periods(hand_pose, ax=None, title="", fps=59):
    hand_dis = get_thumb_index_dis(hand_pose)
    mean_hand_dis = np.mean(hand_dis)
    
    norm_factor = np.mean(np.linalg.norm(hand_pose[:, 0, :-1] - hand_pose[:, 5, :-1], axis=1))
...
def single_thumb_index_hand(r_path, l_path, out_dir, fps=59):
    ...
    # r_hf = extract_thumb_index_periods(right_hand_arr, title=f"Thumb-index right hand", fps=r_fps)

    r_hf = extract_thumb_index_periods(right_hand_arr, ax=ax[0], title=f"Thumb-index right hand", fps=fps)

    l_dt = joblib.load(l_path)
    
    left_hand_arr = preprocess_landmarks(l_dt)[:, 0, :, :]
    # l_hf = extract_thumb_index_periods(left_hand_arr, title=f"Thumb-index left hand", fps=l_fps)

    l_hf = extract_thumb_index_periods(left_hand_arr, ax=ax[1], title=f"Thumb-index left hand", fps=fps)
    ...
    return list(r_hf) + list(r_hf2) + list(l_hf) + list(l_hf2) 
