...
def load_pose_data(pose_2d_pth, pose_3d_pth):
    ...
    # === Normalize 2D pose ===
    root_joint_2d = 11  # e.g., mid-hip
    scale_joint_a_2d, scale_joint_b_2d = 11, 12  # left/right hip
    root_xy = pose_2d[:, root_joint_2d, :2]
    pose_2d[:, :, :2] -= root_xy[:, None, :]
    hip_dist_2d = np.linalg.norm(pose_2d[:, scale_joint_a_2d, :2] - pose_2d[:, scale_joint_b_2d, :2], axis=1)
    hip_dist_2d[hip_dist_2d == 0] = 1
    pose_2d[:, :, :2] /= hip_dist_2d[:, None, None]

    # === Normalize 3D pose ===
    root_joint_3d = 0  # Human3.6M format: usually pelvis = joint 0
    scale_joint_a_3d, scale_joint_b_3d = 0, 1  # e.g., pelvis and left hip

    # Root-center
    root_3d = pose_3d[:, root_joint_3d:root_joint_3d+1, :]  # shape (N, 1, 3)
    pose_3d = pose_3d - root_3d  # broadcast subtraction

    # Scale normalization by hip length
    bone_len = np.linalg.norm(
        pose_3d[:, scale_joint_a_3d, :] - pose_3d[:, scale_joint_b_3d, :], axis=1, keepdims=True
    )
    bone_len[bone_len == 0] = 1  # avoid divide-by-zero
    pose_3d /= bone_len[:, None, :]  # broadcast scale normalization
...
def pose_features_extract(pth_2d, pth_3d, fps=30, plot_results=False, save_fig_pth=None):
