...
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport, limit=-1,
                     downsample=1, size=5, input_video_path=None, com_reconstrcution=False, input_video_skip=0, reset_fps=False):
    """
    TODO
    ...
    sz = keypoints_metadata['num_joints']
    index = [i for i in np.arange(sz)]

    def update_video(i):
        nonlocal initialized, image, lines, points, sz
            ...
            for j, j_parent in zip(index, parents):
                ...
                if com_reconstrcution:
                    for k, pose in enumerate(poses):
                        pos = pose[i]
                        lines_3d[0][j + k*(sz-1) - 1][0].set_data_3d([pos[j, 0], pos[j_parent, 0]], 
                                [pos[j, 1], pos[j_parent, 1]], [pos[j, 2], pos[j_parent, 2]])
                        # lines_3d[0][j + k*(sz-1) - 1][0].set_xdata([pos[j, 0], pos[j_parent, 0]])
                        # lines_3d[0][j + k*(sz-1) - 1][0].set_ydata([pos[j, 1], pos[j_parent, 1]])
                        # lines_3d[0][j + k*(sz-1) - 1][0].set_3d_properties([pos[j, 2], pos[j_parent, 2]], zdir='z')
                else:
                    for n, ax in enumerate(ax_3d):
                        pos = poses[n][i]
                        lines_3d[n][j - 1][0].set_data_3d([pos[j, 0], pos[j_parent, 0]], 
                                [pos[j, 1], pos[j_parent, 1]], [pos[j, 2], pos[j_parent, 2]])
