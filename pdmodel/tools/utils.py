...
def generate_2d_result(file_pt, video_path="../fix_post/", save_out_video=True,
                    output_folder="../final_results", save_results=True, save_path=None):
    
    print("Loading: {}".format(file_pt))
    pose_results_ls = np.load(file_pt, allow_pickle=True)
    processed_pose_result_ls = []
    ...
    if save_out_video:
        ...
        videoWriter = cv2.VideoWriter(os.path.join(output_folder, f'vis_gait_{os.path.basename(video_path)}'),
                                      fourcc, fps, size)
        i = 0
        _pose = None
        f = 0
        ...
        while _pose is None:
            if pose_results_ls[f]:
                _pose = pose_results_ls[f]
                a = np.argmax([np.sum(p["bbox"][-2:] for p in _pose)])
                if _pose[a]["keypoints"][16, 1] < _pose[a]["keypoints"][0, 1]:
                    flip = True
            f += 1
        ...
        while (cap.isOpened()):
            ...
            if not flag:
                break
            result = None
            pose_results = pose_results_ls[i]
            if flip:
                img = cv2.flip(img, 0)
            if pose_results:
                a = np.argmax([np.sum(pose["bbox"][-2:] for pose in pose_results)])
                if flip:
                    for p in range(17):
                        y = size[1] - 1 - pose_results[a]["keypoints"][p, 1]
                        pose_results[a]["keypoints"][p, 1] = y

                result = {'bbox': pose_results[a]["bbox"], "keypoint": pose_results[a]["keypoints"][:17, :]}
                for p in range(17):
                    x = pose_results[a]["keypoints"][p, 0]
                    y = pose_results[a]["keypoints"][p, 1]
                    img = cv2.circle(img, (int(x), int(y)), radius=5, color=(0, 0, 255), thickness=-1)

                videoWriter.write(img)
                processed_pose_result_ls.append(result)
        ...
        for pose_results in pose_results_ls:
            if pose_results:
                result = {'bbox': pose_results[0]["bbox"], "keypoint": pose_results[0]["keypoints"][:17, :]}
                processed_pose_result_ls.append(result)
            else:
                processed_pose_result_ls.append(result)
    ...
    if save_results:
        if save_path:
            np.save(save_path, processed_pose_result_ls)
        else:
            np.save(output_folder + f'/2d_{os.path.basename(video_path)[:-4]}.npy', processed_pose_result_ls)
    else:
        return processed_pose_result_ls
